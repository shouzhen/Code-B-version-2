...
void setquantity() {
    for(map<string,Stock*>::iterator it = security.begin(); it != security.end(); it++) {
        quantity[it->first] = 0;
        for (int i=0; i<security[it->first]->asks.size(); i++)
            quantity[it->first] += security.find(it->first)->second->asks[i].number;
    }
}
        ...
        if (token == "MY_SECURITIES_OUT"){
            ...
            for (int i=1; i<info.size()-1; i+=3){
                ...
                if (security.find(info[i])->second->owned!=0){
                    ...
                    security.find(info[i])->second->selling = false;
            }
        }
    }
//    for(map<string,Stock*>::iterator it = security.begin(); it != security.end(); it++) {
    ...
    for(map<string,Stock*>::iterator it = security.begin(); it != security.end(); it++) {
          ...
          //cout << security.find(it->first)->second->buyscore << endl;
            if (security.find(it->first)->second->buyscore >= maximum) {
                ...
                if (seconds<30 && security.find(it->first)->second->owned==0)
                    continue;
                if (security.find(it->first)->second->selling)
                    continue;
...
//          cout << security.find(it->first)->second->sellscore << endl;
        ...
        if (maximum>minimum){
            ...
            security.find(minsell)->second->selling = true;
